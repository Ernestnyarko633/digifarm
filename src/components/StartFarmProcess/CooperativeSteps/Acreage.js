...
const Acreage = ({  name,
  ...
  order,
  selectedType}) => {
  const { barrier, cooperativeName, coopImg, selectedCooperativeType, setCooperativeName } = useStartFarm();
  ...
  const  initialValues = {
    users: [{ email: user.email, acreage: "" }, { email: "", acreage: "" }]
  };
            ...
            <Formik initialValues={initialValues} >
                {({values, errors, handleChange, handleBlur, touched}) => (
                          ...
                          >
                            <Box borderRightWidth={1} pr={8} borderRightColor="gray.300">
                              <Text color="gray.500">
                                Member left:{" "}
                                <Text as="span" fontWeight={900} color="black">
                                  {(selectedCooperativeType?.maxMember) - values.users.length} {`(${selectedCooperativeType?.maxMember})`}
                                </Text>
                              </Text>
                            </Box>
                            <Box ml={{ md: 16 }}>
                              <Text color={acres >= selectedCooperativeType?.minAcre && acres <= barrier ? 'cf.400' :acres > barrier ? 'red.dark': "gray.500"}>
                                Meet the minimum acreage:{" "}
                                <Text as="span" fontWeight={900} color={acres >= selectedCooperativeType?.minAcre && acres <= barrier ? 'cf.400' : acres > barrier ? 'red.dark':  "black"}>
                  ...
                  <form>
                        ...
                        <>
                          {values?.users?.map((value, i) => <CooperativeMemberCard setAcres={setAcres} acres={acres} isDisabled={i === 0 ? true : false}  member={i + 1} errors={errors?.thirdPartyRef?.[i]} touched={touched?.thirdPartyRef?.[i]} name={`users.${i}.`} values={values?.users} value={value} key={i} onBlur={handleBlur} onChange={handleChange} remove={remove} barrier={barrier} />)} 
                          <Box mb={10}>
                                <Button
                                  ...
                                  isDisabled={values.users?.length === selectedCooperativeType?.maxMember}
